  ...
  return (
    <div className={`space-y-4 w-full text-center ${isKeyboardOpen ? 'px-2' : 'px-4 sm:px-0'}`}>
      <div className={isKeyboardOpen ? 'mb-3' : 'mb-6'}>
        <h2 className={`font-bold text-gold mb-1 ${
          isKeyboardOpen ? 'text-lg' : 'text-2xl sm:text-3xl'
        }`}>
          Qual seu nome?
        </h2>
      </div>
      ...
      {showExpressFlow && onExpressFlow && !isKeyboardOpen && (
        <div className="mt-6 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
          <button
            ...
            className="w-full bg-blue-500 hover:bg-blue-600 text-white py-2 px-3 rounded-lg font-medium text-sm"
          >
      ...
      <Controller
        ...
        render={({ field }) => (
          <RadioGroup
            ...
            value={field.value}
            className={`space-y-2 max-w-sm mx-auto w-full ${isKeyboardOpen ? 'space-y-1' : 'space-y-3'}`}
          >
            {revenueOptions.map((option, index) => (
              <label 
                ...
                className={`flex items-center rounded-lg border border-gold/20 bg-gray-800/30 hover:bg-gray-700/70 hover:border-gold/50 cursor-pointer transition-colors touch-manipulation ${
                  isKeyboardOpen ? 'p-2 min-h-[2.5rem]' : 'p-4 min-h-[3.5rem]'
                }`}
                htmlFor={`r${index+1}`}
              >
                <RadioGroupItem 
